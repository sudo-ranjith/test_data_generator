...
column_names = ["admin", "frontDesk", "technologist"]
...
try:
    ...
    with psycopg2.connect(CONN_STRING, cursor_factory=RealDictCursor) as conn_pg:
        with conn_pg.cursor() as cur_pg:
            for data in range(column_names):
                user_id = uuid.uuid4()     
                created_by = "bot"
                ...
                role_name = column_names[data]
 
                QRY = f"insert into {TABLE_NAME} role_id, created_by, created_dt, updated_by, updated_dt, role_name  VALUES ('{role_id}','{created_by}',  '{created_dt}','{updated_by}', '{updated_dt}', '{role_name}')"
                cur_pg.execute(QRY)
